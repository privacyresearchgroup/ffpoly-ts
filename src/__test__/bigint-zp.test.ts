...
import { BigIntIntegers } from '../bigint-integers'

import { extendIntegers } from '..'

const NBI = new BigIntIntegers()
const XNBI = extendIntegers(NBI)
const serializer = new inthelper.SerializationHelpers(NBI)

describe('Test JSBI implementation of modular arithmetic', () => {
    const p25519 = NBI.BigInt('57896044618658097711785492504343953926634992332820282019728792003956564819949')
    const p25519m1 = NBI.BigInt('57896044618658097711785492504343953926634992332820282019728792003956564819948')
    ...
    test('integer serialization', () => {
        const n2 = NBI.BigInt('1976943448883713') // 1n + 2n * 2n ** 8n + 3n * 2n ** 16n + 4n * 2n ** 24n + 5n * 2n ** 32n + 6n * 2n ** 40n + 7n * 2n ** 48n

        const expectedBytes = [1, 2, 3, 4, 5, 6, 7, 0]
        expect(serializer.toLittleEndian(n2)).toEqual(new Uint8Array(expectedBytes))

        const m707 = NBI.BigInt(-707)
        ...
        expect(NBI.equal(m707FromBE, m707)).toBeTruthy()
        expect(NBI.equal(m707FromLE, m707)).toBeTruthy()
    })
    test('Modular addition', () => {
        ...
        expect(result.toString()).toEqual(NBI.BigInt(3).toString())
    })
    ...
    test('Modular subtraction', () => {
        ...
        expect(result.toString()).toEqual(NBI.BigInt(23).toString())
    })
    test('Modular negation', () => {
        ...
        expect(result).toEqual(NBI.BigInt(10))
    })
    ...
    test('Modular multiplication', () => {
        ...
        expect(result.toString()).toEqual(NBI.BigInt(72).toString())
    })

    test('Modular inverse', () => {
        const modulus = NBI.BigInt(17)
        const zn = new ZpField<bigint>(NBI, modulus)
        const expected = [1, 9, 6, 13, 7, 3, 5, 15, 2, 12, 14, 10, 4, 11, 8, 16].map(NBI.BigInt)

        for (let i = 1; NBI.LT(i, modulus); ++i) {
            const a = NBI.BigInt(i)
    ...
    test('Modular divide', () => {
        const zp = new ZpField<bigint>(NBI, p25519)
    ...
    test('modular exponentiation', () => {
        const p = p25519
        const one = NBI.BigInt(1)
        const minusOne = p25519m1
        const zp = new ZpField<bigint>(NBI, p)

        const b2 = zp.fromNumber(2)
        const b3 = zp.fromNumber(3)
        const b5 = zp.fromNumber(5)

        const s = NBI.divide(p25519m1, NBI.BigInt(2))
    ...
    test('gcd', () => {
        const seven = XNBI.xgcd(
            NBI.multiply(NBI.BigInt(7), p25519),
            NBI.multiply(
                NBI.BigInt(7),
                NBI.BigInt('92298539445058780156311400358427006136490327688468392976721825472309537485619')
            )
        )

        const seven2 = XNBI.xgcd(NBI.BigInt(-707), NBI.BigInt(1001))
        console.log('seven: ', seven)
        console.log('seven2: ', seven2)

        const integerCombo = NBI.add(
            NBI.multiply(seven.x, NBI.multiply(NBI.BigInt(7), p25519)),
            NBI.multiply(
                seven.y,
                NBI.multiply(
                    NBI.BigInt(7),
        ...
        expect(seven.gcd.toString()).toEqual(NBI.BigInt(7).toString())
        expect(seven2.gcd.toString()).toEqual(NBI.BigInt(7).toString())

        expect(integerCombo).toEqual(seven.gcd)

        // console.log(xgcd(0n, 100n))
        // console.log(xgcd(1000n, 0n))
        expect(XNBI.xgcd(NBI.BigInt(0), NBI.BigInt(100)).gcd.toString()).toEqual(NBI.BigInt(100).toString())
        expect(XNBI.xgcd(NBI.BigInt(0), NBI.BigInt(0)).gcd.toString()).toEqual(NBI.BigInt(0).toString())
    })
    ...
    test('serialization', () => {
        const zp = new ZpField<bigint>(XNBI, p25519)
        ...
        const n3 = NBI.BigInt('1976943448883713') // 1n + 2n * 2n ** 8n + 3n * 2n ** 16n + 4n * 2n ** 24n + 5n * 2n ** 32n + 6n * 2n ** 40n + 7n * 2n ** 48n
